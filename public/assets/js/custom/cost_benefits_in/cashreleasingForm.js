...
function countSelectedCashReleasing(oForm) {
    var i, n = 0;
    var oElement;
    ...
    $("#countCashReleasingSelect").text(n+" selected");
    //console.log(n);
    if (n >= 1) {
        $("#help_cashreleasing").attr('hidden', 'hidden');
        return true;
    }
    else {
        $("#help_cashreleasing").removeAttr('hidden');
        return false;
    }
}

function checkCashReleasingInput(){
    var ret = true;
    $("#cashreleasing_input input").each(function(){
        var val = $(this).val();
